const DomElement = (function () {
    ...
    const GSD_twoPlayerOption_inputs = document.querySelectorAll(".two_players input");
    const GSD_playerVsComputerOption_inputs = document.querySelectorAll(".player_vs_ai input");
...
class ComputerAI extends GameBoard {
    ...
    static computerFillCell(computerPlayerObj) {
        ...
        setTimeout(() => {
            this._gameBoard[ComputerAI.findBestMove(this._gameBoard)] = computerPlayerObj.mark;
            uiController.rightSectionController.renderGameBoard(this._gameBoard);
...
const GameLogic = (function () {
    ...
    const startNewGame = function (gameType) {
        ...
        uiController.leftSectionController.updateLogPlayerNames(player1.name, player2.name);
        uiController.rightSectionController.DisplayNameAndIcon(player1.name, player2.name);
    ...
    const endRound = function (winner) {
        if (winner) {
            ...
            uiController.leftSectionController.updateScore(player1.score, player2.score);
        }
...
const uiController = (function () {
    ...
    const startingDialogController = {
        ...
        startOrExitGame: function (target) {
            if (target.id === "start") {
                if (
                    (DomElement.GSD_twoPlayerOption_inputs[0].value != DomElement.GSD_twoPlayerOption_inputs[1].value &&
                        DomElement.GSD_twoPlayerOption_inputs[0].value.toLowerCase() != "computer" &&
                        DomElement.GSD_twoPlayerOption_inputs[1].value.toLowerCase() != "computer") ||
                    ...
                    document.querySelector(".same-name-err").style.display = "inline-block";
    ...
    const leftSectionController = {
        ...
        updateLogPlayerNames: function (player1, player2) {
            [...DomElement.leftSectionLog.children].forEach((item, index) => {
                [...[...item.children][0].children][0].textContent = [player1, player2][index];
            });
        },
        updateScore: function (player1Score, player2Score) {
            [...DomElement.leftSectionLog.children].forEach((item, index) => {
                [...[...item.children][1].children][0].textContent = [player1Score, player2Score][index];
            });
        },
    ...
    const rightSectionController = {
        ...
        DisplayNameAndIcon: function (player1, player2) {
            ...
            if (player2.toLowerCase() === "computer") {
                [...[...DomElement.turns.children][1].children][0].classList.add("computer");
            }
        },
        ...
        displayRoundEndDialog: function (winner) {
            DomElement.roundEnded_dialog.style.display = "flex";
            [...[...DomElement.roundEnded_dialog.children][0].children].forEach((item) => {
                //winning msg for single player mode
                if (GameLogic.currentPlayer().name == "Computer" || GameLogic.getNextPlayer().name == "Computer") {
                    if (item.tagName === "H1") {
...
const DomListener = (function () {
    ...
    const gridCellClick = (e) => {
        if (e.target.closest(".board_column")) {
            ...
            const cellIndex = Array.from(DomElement.board.children).indexOf(clicked_column);
...
const DomEvent = (function () {
    DomElement.gameStartingDialog.addEventListener("click", DomListener.startExitGame);
    ...
    DomElement.roundEnded_dialog.addEventListener("click", DomListener.roundEnded_dialogClick);
})();
